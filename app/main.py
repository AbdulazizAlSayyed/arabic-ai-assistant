"""
Arabic Virtual AI Assistant - Main Streamlit Application
Multi-task NLP assistant with Traditional + LLM approaches
"""
...
# Header
st.markdown('<div class="main-header"><h1>🤖 Arabic AI Assistant</h1><p>Multi-Task NLP Assistant with Traditional + Modern Approaches</p></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    - 😊 **Sentiment Analysis** (CAMeL BERT + ML + Rule-based)
    - 🗣️ **Dialect Identification** (CAMeL Tools + Traditional NLP)
    - 📝 **Text Summarization** (HuggingFace mT5)
    - 🧠 **RAG Question Answering** (Embeddings + Retrieval)
    """)
    
    st.markdown("---")
    st.markdown("## 📊 System Stats")
    st.metric("Tasks Supported", "5")
    st.metric("Models Used", "7+")
    st.metric("Languages", "Arabic, English")
    
    st.markdown("---")
    st.markdown("## 🔧 Technologies")
    st.markdown("""
    - **Traditional NLP**: Rule-based Patterns, TF-IDF
    - **CAMeL Tools**: Dialect ID, Sentiment BERT
    - **Transformers**: MarianMT, mT5
        ...
        with st.spinner("Processing..."):
            ...
            if task == "🌍 Machine Translation":
                ...
                with col1:
                    ...
                    st.info(user_input)
                    st.caption(f"🔤 Detected: {'Arabic' if lang == 'ar' else 'English'}")
                with col2:
...
st.markdown("---")
st.markdown(
    "<p style='text-align: center; color: gray;'>🤖 Arabic Virtual AI Assistant | Built with ❤️ using CAMeL Tools + HuggingFace + Streamlit</p>",
    unsafe_allow_html=True
)